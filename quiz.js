const quizData = [
  ...
  {
    questions: "Which player was the highest goal scorer in the 2022 world cup?",
    a: "Kylian Mbappe",
    b: "Harry Kane",
    c: "Lionel Messi",
    d: "Enzo Fernandez",
  ...
  {
    questions:
      "How many goals did Ronaldo score in the 2022 world cup?",
    a: "1",
    b: "2",
    c: "3",
    d: "4",
  ...
  if (time >= 0) {
    ...
    if (seconds == 0 || seconds < 10) {
      ...
      if (minutes == 0 && seconds == 0) {
        if (score < 5) {
          countdwn.remove();
          quiz.innerHTML = `<h2>OLODO ${name}!! You answered ${score}/${quizData.length} questions  correctly. Try again</h2><button onclick="location.reload()">RETAKE THE QUIZ</button>`;
        } else {
          countdwn.remove();
          quiz.innerHTML = `<h2>AGBA ${name}!! You answered ${score}/${quizData.length} questions  correctly. Well done</h2><button onclick="location.reload()">RETAKE THE QUIZ</button>`;
        }
      }
    } else {
  ...
  if (answer) {
    ...
    if (answer) {
      if (currentQuestion < quizData.length) {
        ...
        if (score < 5) {
          countdwn.remove();
          quiz.innerHTML = `<h2>OLODO ${name}!! You answered ${score}/${quizData.length} questions  correctly. Try again</h2>
          <button onclick="location.reload()">RETAKE THE QUIZ</button>`;
        } else {
          countdwn.remove();
          quiz.innerHTML = `<h2>AGBA ${name}!! You answered ${score}/${quizData.length} questions  correctly. Well done</h2>
          <button onclick="location.reload()">RETAKE THE QUIZ</button>`;
        }
      }
    }
